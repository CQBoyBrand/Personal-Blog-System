...
export const getZhLang = () => {
  const langFiles = import.meta.glob('@/locales/zh/*ts', { import: 'default', eager: true }) as FileParams;
...
export const getEnLang = () => {
  const langFiles = import.meta.glob('@/locales/en/*ts', { import: 'default', eager: true }) as FileParams;
  const result = handleFileList(langFiles);
  return result;
};
